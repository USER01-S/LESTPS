...
Point::Point() : coord_x(0.0f), coord_y(0.0f) {
    cout << "Constructeur Point() : point initialisé à (0,0)" << endl;
}

// Constructeur avec paramètres : initialise aux coordonnées données
Point::Point(float x, float y) : coord_x(x), coord_y(y) {
    cout << "Constructeur Point(float, float) : point initialisé à ("
         << x << "," << y << ")" << endl;
}

// Constructeur de copie : recopie les coordonnées d'un autre point
Point::Point(const Point& p) : coord_x(p.coord_x), coord_y(p.coord_y) {
    cout << "Constructeur de copie Point(const Point&)" << endl;
}
